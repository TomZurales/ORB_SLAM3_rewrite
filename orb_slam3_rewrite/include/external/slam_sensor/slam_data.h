...
private:
  cv::Mat image_left;
  cv::Mat image_right;
  std::vector<std::vector<float>> imu_readings;

public:
  SlamData() { imu_readings = std::vector<std::vector<float>>(); }
  void setImageLeft(cv::Mat image) { image_left = image; }
  void setImageRight(cv::Mat image) { image_right = image; }
  void addImuReading(std::vector<float> imuReading) { imu_readings.push_back(imuReading); }
